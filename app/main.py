# app/main.py
import os
from pathlib import Path  # مكتبة للتعامل مع مسارات الملفات بذكاء
from dotenv import load_dotenv
from services.ingestion_service import IngestionService
from services.query_service import QueryService

# --- ديناميكية قراءة ملف الـ .env من المسار الصحيح ---
# بيجيب مسار الفولدر الرئيسي للمشروع (Root) بره فولدر app
base_dir = Path(__file__).resolve().parent.parent
env_path = base_dir / '.env'

# فحص المسار غصب عن الويندوز (سواء كان .env أو مضاف له .txt بالخطأ)
if env_path.exists():
    load_dotenv(dotenv_path=env_path)
else:
    load_dotenv(dotenv_path=base_dir / '.env.txt')
# ----------------------------------------------------

if __name__ == "__main__":
    # بايثون هيقرا الـ API Key من البيئة من غير ما نكتبه صراحة هنا
    MY_GROQ_KEY = os.getenv("GROQ_API_KEY") 
    
    # حائط صد للتأكد من أن الـ Key مقروء بنجاح قبل بدء السيستم
    if not MY_GROQ_KEY:
        print("❌ ERROR: GROQ_API_KEY not found in your .env file!")
        print("💡 Please create a .env file in the root directory and add: GROQ_API_KEY=your_key")
        exit(1)
    
    print("🚀 --- PHARMA-FLOW SYSTEM INITIALIZING (GROQ MODE) --- 🚀")

    # 1. المرحلة الأولى: سحب الداتا وملء الـ RAG
    print("\n🛠️ STEP 1: Ingesting Medical Data...")
    ingestor = IngestionService()
    ingestor.ingest_pubmed("Paracetamol drug interactions and alternatives") 

    # 2. المرحلة الثانية: تشغيل الأجنت والخدمات
    print("\n🤖 STEP 2: Initializing AI Pharma Agent...")
    service = QueryService(api_key=MY_GROQ_KEY)

    # --- السيناريوهات (Test Cases) ---

    # حالة رقم 1: نقص مخزون مع يقين عالي (هنشوف الـ RAG)
    case_1 = {
        "sku": "PARA-500",
        "sku_name": "Paracetamol",
        "location_type": "hospital",
        "stock": 0,             # مخزون صفر
        "on_order": 0,
        "forecast_demand": 500, # طلب عالي جداً
        "expiry_days": 60,
        "available_stock": 0,
        "confidence_low": 90,   
        "confidence_high": 95   
    }

    # حالة رقم 2: بيانات متخبطة جداً (هنشوف الـ Human Review)
    # هنا الفجوة 90% (95 - 5) والطلب 100، يعني الـ Gap أكبر من 30% بكتير
    case_2 = {
        "sku": "ASP-100",
        "sku_name": "Aspirin",
        "location_type": "warehouse",
        "stock": 10,
        "on_order": 0,
        "forecast_demand": 100,
        "expiry_days": 100,
        "available_stock": 10,
        "confidence_low": 1,    # يقين 1%
        "confidence_high": 99   # يقين 99% (فجوة 98 وحدة!)
    }

    # --- التشغيل الفعلي ---

    print("\n🔥 RUNNING SCENARIO 1: Clinical Shortage (RAG Enabled)")
    service.execute_agent_decision(case_1)

    print("\n🔥 RUNNING SCENARIO 2: Data Uncertainty (Safe AI Mode)")
    service.execute_agent_decision(case_2)

    print("\n✅ --- SPRINT 2 DEMO COMPLETED SUCCESSFULLY --- ✅")