...
class AuditorAgent(BaseAgent):
    """
    Auditor Agent â€” deterministic rule engine and Safe AI Guardrail.
            ...
            if forecast_demand < 0:
                rules.append(
                    AuditRuleResult(
                        ...
                        f"forecast_demand is negative ({forecast_demand}); workflow remains in HOLD_MONITOR.",
                    )
                )
    ...
    def _generate_audit_summary(self, ops_decision, risk_assessment, approved, audit_status, rule_results) -> dict:
        return self._build_grounded_audit_summary(ops_decision, risk_assessment, approved, audit_status, rule_results)

    def _build_grounded_audit_summary(self, ops_decision, risk_assessment, approved, audit_status, rule_results) -> dict:
        failed_rules = [r.rule_name for r in rule_results]
        failed_rules_text = ", ".join(failed_rules) if failed_rules else "No rule violations detected"
        return {
            "reasoning": (
                f"Audit status is {audit_status}; approved={approved}. "
                f"Failed rules: {failed_rules_text}. Risk level is {risk_assessment.risk_level}, "
