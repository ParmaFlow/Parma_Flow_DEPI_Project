...
class OpsAgent(BaseAgent):
    ...
    def _calculate_inventory_gap(self, item_data: dict, reorder_point: int, safety_stock: int = 0) -> int:
        ...
        """
        if self._is_demand_paused(item_data) and (item_data.get("available_stock", 0) or 0) > 0:
            return 0
    ...
    def _needs_reorder(self, item_data: dict, reorder_point: int, safety_stock: int = 0) -> bool:
        ...
        reported gap figure — eliminating the green-light-on-stockout bug.
        """
        if self._is_demand_paused(item_data) and (item_data.get("available_stock", 0) or 0) > 0:
            return False
    ...
    def _build_grounded_operational_summary(self, item_data: dict, **computed) -> dict:
        ...
        if action == ActionType.REORDER.value:
            recommendation = (
                ...
                f"{inventory_gap:,}-unit inventory gap and restores the safety stock buffer."
            )
            reasoning = (
                f"{sku_name} is in {inventory_status} inventory status with {available_stock:,} units "
                f"available against a reorder point of {reorder_point:,} units, producing a "
                f"{inventory_gap:,}-unit supply gap. The procurement trigger threshold has been breached, "
                f"so the deterministic action is REORDER, not MONITOR. The recommended quantity is "
                f"{recommended_qty:,} units, rounded from the gap to preserve clinical formulary "
                f"continuity while accounting for the {safety_stock:,}-unit safety stock buffer and "
                f"{lead_time}-day lead time buffer. Current stock cover indicates {runway_text}, so "
            ...
            reasoning = (
                ...
                f"deterministic action is HOLD_MONITOR, inventory_gap is {inventory_gap:,} units, "
                f"and recommended_qty is {recommended_qty:,} units. Current stock cover is exactly "
            ...
            reasoning = (
                f"{sku_name} is in {inventory_status} inventory status with {available_stock:,} units "
                f"available against a reorder point of {reorder_point:,} units and a computed "
                f"inventory gap of {inventory_gap:,} units. Because there is no positive supply gap, "
                "the deterministic action is MONITOR and the recommended quantity is 0 units. "
