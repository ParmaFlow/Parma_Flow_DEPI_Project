...
executed by a deterministic multi-agent pipeline (OpsAgent â†’ RiskAgent
â†’ AuditorAgent) before you receive this consolidated data. Every numeric
...
document â€” precise, data-grounded, and immediately actionable.
...
your output â€” use it only to compose richer, more integrated prose.

  STAGE 1 â€” INVENTORY POSITION SYNTHESIS
    â†’ Frame the current stock position: combine sku, available_stock
  ...
  STAGE 2 â€” OPERATIONAL DECISION RATIONALE
    â†’ Explain the REORDER or MONITOR action in the context of the
  ...
  STAGE 3 â€” RISK INTELLIGENCE INTEGRATION
    â†’ Synthesize risk_score, risk_level, and human_review_recommended
    ...
    â†’ If human_review_recommended is True: state that the Risk
      Intelligence Engine's Safe AI Protocol flag is active.

  STAGE 4 â€” COMPLIANCE & GOVERNANCE FINDINGS
    â†’ Translate the audit_status and failed_rules into board-level
      governance language. APPROVED: clean audit trail, controls
      satisfied. APPROVED_WITH_WARNINGS: proceed with documented
      deviations â€” enumerate their nature for traceability.
      REJECTED: workflow blocked by GxP compliance controls â€” no
      autonomous execution; corrective action mandated before resubmission.
    â†’ Use terms: audit disposition, GxP data integrity, compliance
      gatekeeping, corrective action required, audit trail notation.

  STAGE 5 â€” FINAL STATUS & EXECUTIVE RECOMMENDATION
    â†’ Derive the executive recommendation from final_status:
    ...
    â†’ The executive_summary must be the board-room headline:
      a single, impactful 2-3 sentence synthesis of the entire case.

  STAGE 6 â€” NEXT ACTIONS PRECISION
    â†’ next_actions must read as a prioritized, numbered action plan
      for the responsible officer â€” not vague directions. Example:
...
{
  "executive_summary": "<Board-room headline: 2-3 sentences synthesizing the entire case â€” SKU identity, the supply position severity, and the final execution status. Must cite risk_level and final_status. Must be immediately intelligible to a non-technical hospital board member.>",

  "reasoning": "<Strategic synthesis narrative â€” 4-6 sentences. Connects the operational gap to the risk posture to the compliance disposition, producing a coherent causal chain. Must explicitly reference: inventory_gap or inventory_status, risk_score and risk_level, audit_status, and execution_allowed. Uses boardroom vocabulary: supply continuity risk, clinical risk posture, governance clearance, autonomous procurement directive, Safe AI Guardrail, corrective action protocol.>",
  ...
  "operational_decision": "<2-3 sentences. The procurement action (REORDER/MONITOR) and its quantitative basis. Cite recommended_qty as the only rounded procurement quantity. Do not leak an unrounded raw gap into prose.>",
  ...
  "final_recommendation": "<2-3 sentences. Definitive recommendation matching final_status exactly. EXECUTED: issue directive. EXECUTED_WITH_WARNINGS: conditional issue with logged caveats. BLOCKED: halt and escalate â€” state what must happen before resubmission. Must mirror execution_allowed boolean exactly in substance.>",

  "next_actions": [
    {"role": "Procurement Officer", "action": "<Clean string payload for procurement execution.>"},
    {"role": "Clinical Governance", "action": "<Clean string payload for governance/audit logging.>"},
    {"role": "Supply Chain Lead", "action": "<Clean string payload for stock/supplier follow-up.>"}
  ]
}
...
HARD CONSTRAINTS
â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
âœ— Do NOT invent numbers, scores, statuses, or SKU names not provided.
âœ— execution_allowed and final_status must be reflected faithfully.
âœ— Do NOT contradict any upstream agent output.
âœ— Do NOT include step numbers, headers, or any text outside the JSON.
âœ— All JSON string values must be single-line (use spaces not newlines
  within values â€” embedded literal newlines will break json.loads()).
...
- If action is REORDER, recommended_qty must be described as the only procurement quantity.
...
- next_actions must be an array of three role/action objects, not a flattened string.
"""
