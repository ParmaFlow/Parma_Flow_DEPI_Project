...
class ReportAgent(BaseAgent):
    """
    Report Agent â€” merges upstream outputs into a professional executive
    ...
    def _build_grounded_executive_summary(
        ...
        if expiry_override:
            ...
            procurement_payload = (
                f"Quarantine expiring {sku} stock and initiate urgent disposal-and-replacement "
                "workflow within 2 hours."
            )
        elif action == "REORDER":
            operational_sentence = (
                f"A procurement requisition for {qty:,} units has been generated as the single "
                "rounded supply requirement."
            )
            procurement_payload = (
                f"Issue emergency procurement requisition for {qty:,} units of {sku} within 4 hours."
            )
        else:
            operational_sentence = (
                f"No procurement requisition has been generated; the rounded supply requirement is {qty:,} units."
            )
            procurement_payload = (
                f"Continue monitoring {sku}; do not issue a purchase order unless a rounded "
                "supply requirement is approved."
            )
        ...
        next_actions = [
            {
                ...
                "action": procurement_payload,
            },
            {
                "role": "Clinical Governance",
                "action": (
                    f"Record {risk_level} risk posture, audit status {audit_status}, "
                    f"and blocking_errors={blocking_errors} in the decision log."
                ),
            },
            {
                "role": "Supply Chain Lead",
                "action": (
                    "Recheck stock position after supplier confirmation or the next inventory refresh; "
                    f"use {qty:,} units as the rounded quantity reference."
                ),
            },
        ]
        ...
        return {
            "executive_summary": (
                f"{sku} is classified as {inventory_status} with {risk_level} risk. "
                f"Recommended action is {action}, rounded quantity is {qty:,} units, "
                f"final status is {final_status}, and execution_allowed is {execution_allowed}."
            ),
            "reasoning": (
                f"The operational layer produced action={action} and rounded quantity={qty:,} units. "
            ...
            "inventory_summary": (
                ...
                f"rounded quantity reference of {qty:,} units."
            ),
            "operational_decision": (
