...
def _resolve_api_key() -> str:
    ...
    if not api_key:
        raise RuntimeError("GROQ_API_KEY is required for workflow execution.")
    return api_key
...
def build_dashboard_response(state: Any, role: UserRole) -> WorkflowDashboardResponse:
    ...
    report = _normalize_report_payload(report)
    ...
    if operational and operational.get("inventory_gap", 0) > 0:
        alerts.append(
            ActionAlert(
                ...
                message=(
                    f"Order {operational.get('recommended_qty', 0):,} units as the rounded "
                    "supply requirement."
                ),
...
def _next_action_summary(report: Dict[str, Any] | None) -> Dict[str, Any] | None:
    ...
    return {
        ...
        "next_actions": normalize_next_actions(sections.get("next_actions")),
    }


def _normalize_report_payload(report: Dict[str, Any] | None) -> Dict[str, Any] | None:
    if report is None:
        return None
    sections = report.setdefault("report_sections", {})
    sections["next_actions"] = normalize_next_actions(sections.get("next_actions"))
    return report
