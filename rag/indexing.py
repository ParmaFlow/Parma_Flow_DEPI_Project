...
def build_index(file_path: str | Path | None = None) -> tuple[VectorStore, EmbeddingModel]:
    ...
    # C-3 fix — guard #1: file existence check with actionable message.
    ...
    # C-3 fix — guard #2: empty corpus produces a useless index.
    if not documents:
        raise ValueError(
    ...
    logger.info(
        "RAG index built: %d documents → %d chunks → %d-dim vectors",
