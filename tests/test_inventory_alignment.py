...
if "rag.indexing" not in sys.modules:
    rag_indexing_stub = types.ModuleType("rag.indexing")
    rag_indexing_stub.build_index = lambda *args, **kwargs: (None, None)
    sys.modules["rag.indexing"] = rag_indexing_stub

if "rag.retriever" not in sys.modules:
    rag_retriever_stub = types.ModuleType("rag.retriever")
    rag_retriever_stub.retrieve = lambda *args, **kwargs: []
    sys.modules["rag.retriever"] = rag_retriever_stub
...
class InventoryAlignmentTests(unittest.TestCase):
    def test_ops_agent_clopidogrel_stockout_forces_reorder(self):
        ...
        self.assertIn("3,481-unit supply gap", decision.reasoning)
    ...
    def test_report_agent_uses_one_gap_and_one_procurement_quantity(self):
        ...
        sections = " ".join(final.report_sections.values())

        self.assertTrue(final.execution_allowed)
        self.assertEqual(final.final_status, "EXECUTED")
        self.assertIn("3,481-unit supply gap", sections)
    ...
    def test_expiry_dominance_blocks_and_reports_disposal_replacement(self):
        ...
        self.assertIn("urgent disposal", final.report_sections["next_actions"].lower())
